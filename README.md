...
The problem is RLHF's score-maxing game, where it learns to outputs that score high on reward model without actually getting better itself. The DPO addresses this by mathematically reparameterizing the reward function in terms of the policy itself, meaning the RM is implicitly embedded directly within the policy, eliminating the need of separate RM as the math shows the ability to express the optimal reward directly through the trained policy to the reference policy.
...
DPO uses binary cross-entropy loss (BCE) that increases the likelihood of preferred response relative to the rejected one, keeping close to the reference model by penalizing the policy for moving too far from the reference model's distribution on any output, not just the pairs, via KL penalty making the DPO stable.
